...
export async function fetchBlockGroupsInRadius(lat, lng, radiusMiles, year, apiKey) {
  ...
  // Includes full polygon rings so we can detect ANY overlap with the radius circle,
  // not just centroid containment — critical for small radii like 1 mile.
  let bgGeometries = {};
  try {
    let offset = 0;
    const pageSize = 1000;
    ...
    while (keepFetching) {
      const tigerUrl = `https://tigerweb.geo.census.gov/arcgis/rest/services/TIGERweb/tigerWMS_Current/MapServer/10/query?where=STATE%3D'${stateFips}'%20AND%20COUNTY%3D'${countyFips}'&outFields=GEOID,INTPTLAT,INTPTLON&f=json&returnGeometry=true&geometryPrecision=4&resultOffset=${offset}&resultRecordCount=${pageSize}`;
      ...
      if (tigerData?.features) {
        ...
        // If the API signals there are more records, fetch the next page
    ...
    console.warn('TIGER boundary fetch failed, will use all block groups:', e.message);
  }
