...
export function aggregateBlockGroups(bgData, insideGeoids) {
  const totals = {};
  const counts = {}; // for median weighted avg
  const weightedSums = {}; // for medians
  ...
  // Weighted median income approximation using population weights
  const medianVars = ['B19013_001E', 'B19301_001E'];
  for (const mv of medianVars) {
    let weightSum = 0, valSum = 0;
    const popVar = mv === 'B19013_001E' ? 'B17001_001E' : 'B01001_001E';
    for (const geoid of insideGeoids) {
      const row = bgData[geoid];
      if (!row) continue;
      const val = parseFloat(row[mv]);
      const pop = parseFloat(row[popVar]);
      if (isNaN(val) || val < 0 || isNaN(pop) || pop <= 0) continue;
      valSum += val * pop;
      weightSum += pop;
    }
...
export function computeMetrics(raw) {
  ...
  for (const [catKey, cat] of Object.entries(VARIABLES)) {
    ...
    for (const [vKey, vDef] of Object.entries(cat.vars)) {
      let val = 0;
      if (vDef.table) {
        val = parseFloat(raw[vDef.table]) || 0;
      } else if (vDef.tables) {
        val = vDef.tables.reduce((s, t) => s + (parseFloat(raw[t]) || 0), 0);
      }
      if (val < 0) val = null; // Census uses -666666666 for N/A
      out[catKey][vKey] = val;
    }
  }
  return out;
