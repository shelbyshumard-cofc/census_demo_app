...
export const ACS_YEARS = [2023, 2022, 2021, 2020, 2019, 2018, 2017, 2016, 2015, 2014, 2013];
// Note: ACS 2024 5-year estimates are typically released in December 2025.
// Add 2024 back once confirmed available at api.census.gov.
...
export const VARIABLES = {
  ...
  income: {
    ...
    vars: {
      ...
      // C17002 is available at block group level; _002E + _003E = below 1.0 poverty ratio = below poverty line
      // B17001_002E is only available at tract+ level so we use C17002 for radius, B17001 for comparisons
      poverty_pop:       { table: 'C17002_002E', table2: 'C17002_003E', label: 'Population Below Poverty Line', isPovertySum: true },
      poverty_universe:  { table: 'C17002_001E', label: 'Population for Whom Poverty Status Determined', hidden: true },
    }
  },
  education: {
...
export function getAllVarCodes() {
  ...
  for (const cat of Object.values(VARIABLES)) {
    for (const v of Object.values(cat.vars)) {
      ...
      if (v.table2) codes.add(v.table2);
...
export async function fetchBlockGroupsInRadius(lat, lng, radiusMiles, year, apiKey) {
  ...
  // Fetch block group boundaries from Census TIGER for intersection-based radius filtering
  // Uses paginated requests and a timeout to handle large counties reliably
  let bgGeometries = {};
  try {
    // Use a 8-second timeout per page to avoid Vercel function timeouts
    ...
    let offset = 0;
    const pageSize = 500; // smaller pages = more reliable on serverless
    let keepFetching = true;

    while (keepFetching) {
      const tigerUrl = `https://tigerweb.geo.census.gov/arcgis/rest/services/TIGERweb/tigerWMS_Current/MapServer/10/query?where=STATE+%3D+%27${stateFips}%27+AND+COUNTY+%3D+%27${countyFips}%27&outFields=GEOID%2CINTPTLAT%2CINTPTLON&f=json&returnGeometry=true&geometryPrecision=4&resultOffset=${offset}&resultRecordCount=${pageSize}`;

      const tigerRes = await fetchWithTimeout(tigerUrl);
      if (!tigerRes.ok) { keepFetching = false; break; }

      const tigerData = await tigerRes.json();
      if (!tigerData?.features?.length) { keepFetching = false; break; }

      for (const f of tigerData.features) {
        const { GEOID, INTPTLAT, INTPTLON } = f.attributes;
        if (!GEOID) continue;
        const rings = f.geometry?.rings || [];
        // Ensure all coordinates are numbers (TIGER sometimes returns strings)
        const parsedRings = rings.map(ring =>
          ring.map(pt => [parseFloat(pt[0]), parseFloat(pt[1])])
              .filter(pt => !isNaN(pt[0]) && !isNaN(pt[1]))
        ).filter(ring => ring.length >= 3);

        bgGeometries[GEOID] = {
          centroidLat: parseFloat(INTPTLAT),
          centroidLng: parseFloat(INTPTLON),
          rings: parsedRings,
        };
      }

      if (tigerData.exceededTransferLimit && tigerData.features.length === pageSize) {
        offset += pageSize;
      } else {
        keepFetching = false;
      }
    ...
    console.warn('TIGER boundary fetch failed:', e.message);
  }

  // If TIGER failed entirely, fall back to Census geocoder block group centroids
  // This gives us at least the interior points without polygon boundaries
  if (Object.keys(bgGeometries).length === 0) {
    console.warn('TIGER fetch returned 0 geometries — using geocoder block group lookup');
    try {
      const bgUrl = `https://geocoding.geo.census.gov/geocoder/geographies/coordinates?x=${lng}&y=${lat}&benchmark=Public_AR_Current&vintage=Census2020_Current&layers=10&format=json`;
      const bgRes = await fetch(bgUrl);
      const bgData = await bgRes.json();
      const bgs = bgData?.result?.geographies?.['Census Block Groups'] || [];
      for (const bg of bgs) {
        if (bg.GEOID && bg.INTPTLAT && bg.INTPTLON) {
          bgGeometries[bg.GEOID] = {
            centroidLat: parseFloat(bg.INTPTLAT),
            centroidLng: parseFloat(bg.INTPTLON),
            rings: [], // no polygon data in this fallback
          };
        }
      }
    } catch (e2) {
      console.warn('Block group centroid fallback also failed:', e2.message);
    }
  }

  return { stateFips, countyFips, cityName, cityGeoId, bgGeometries };
}
...
export async function fetchBlockGroupACS(stateFips, countyFips, year, apiKey) {
  ...
  for (const chunk of chunks) {
    ...
    if (!res.ok) {
      ...
      // Give a helpful message if the year isn't available yet
      if (res.status === 404 || text.includes('not found') || text.includes('unknown')) {
        throw new Error(`ACS ${year} 5-year data is not yet available from the Census Bureau. Please select an earlier year.`);
      }
      throw new Error(`Census API error: ${res.status} - ${text.substring(0, 200)}`);
    }
    const data = await res.json();
    // Guard: Census sometimes returns an error object instead of an array
    ...
    if (stateIdx < 0 || countyIdx < 0 || tractIdx < 0 || bgIdx < 0) continue;
...
export function computeMetrics(raw) {
  ...
  // Guard against null/undefined raw input (e.g. city lookup returns nothing)
  if (!raw || typeof raw !== 'object') raw = {};
  const out = {};
  for (const [catKey, cat] of Object.entries(VARIABLES)) {
    out[catKey] = {};
    for (const [vKey, vDef] of Object.entries(cat.vars)) {
      if (vDef.isPovertySum) {
        const v1 = parseFloat(raw[vDef.table]);
        const v2 = parseFloat(raw[vDef.table2]);
        const valid1 = !isNaN(v1) && v1 > CENSUS_NA;
        const valid2 = !isNaN(v2) && v2 > CENSUS_NA;
        out[catKey][vKey] = (valid1 || valid2) ? (valid1 ? v1 : 0) + (valid2 ? v2 : 0) : null;
      } else if (vDef.table) {
        ...
        for (const t of vDef.tables) {
          ...
          if (!isNaN(val) && val > CENSUS_NA) { total += val; hasValid = true; }
        }
...
export function buildResultsTable(radiusMetrics, countyMetrics, stateMetrics, cityMetrics) {
  // Ensure all metrics objects have every category key, even if empty
  const safeMetrics = (m) => {
    if (!m || typeof m !== 'object') m = {};
    const safe = {};
    for (const catKey of Object.keys(VARIABLES)) {
      safe[catKey] = (m[catKey] && typeof m[catKey] === 'object') ? m[catKey] : {};
    }
    return safe;
  };

  const rm = safeMetrics(radiusMetrics);
  const cm = safeMetrics(countyMetrics);
  const sm = safeMetrics(stateMetrics);
  const cim = cityMetrics ? safeMetrics(cityMetrics) : null;
  ...
  for (const [catKey, cat] of Object.entries(VARIABLES)) {
    ...
    for (const [vKey, vDef] of Object.entries(cat.vars)) {
      ...
      const r  = rm[catKey][vKey]  ?? null;
      const c  = cm[catKey][vKey]  ?? null;
      const s  = sm[catKey][vKey]  ?? null;
      const ci = cim ? (cim[catKey][vKey] ?? null) : null;

      const rTotal  = rm[catKey][totalPopKey]  || null;
      const cTotal  = cm[catKey][totalPopKey]  || null;
      const sTotal  = sm[catKey][totalPopKey]  || null;
      const ciTotal = cim ? (cim[catKey][totalPopKey] || null) : null;
